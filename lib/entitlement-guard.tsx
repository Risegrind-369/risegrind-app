...
export function EntitlementGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Wait for both auth and RevenueCat to finish loading
    if (authLoading || rcLoading) {
      console.log("[ENTITLEMENT] Waiting for auth/RevenueCat to load");
    ...
    // User is authenticated, NO entitlement, onboarding complete, NOT in paywall → FORCE TO PAYWALL
